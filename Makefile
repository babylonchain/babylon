...
gosec:
	$(DOCKER) run --rm -it -w /$(PROJECT_NAME)/ -v $(CURDIR):/$(PROJECT_NAME) securego/gosec -exclude-generated -exclude-dir /$(PROJECT_NAME)/testutil  -conf /$(PROJECT_NAME)/gosec.json /$(PROJECT_NAME)/...

gosec-local:
	gosec -exclude-generated -exclude-dir $(CURDIR)/testutil  -conf $(CURDIR)/gosec.json $(CURDIR)/...
