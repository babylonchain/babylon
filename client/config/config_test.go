...
// adapted from https://github.com/cosmos/cosmos-sdk/blob/6d32debf1aca4b7f1ed1429d87be1d02c315f02d/client/config/config_test.go
...
func TestCreateClientConfig(t *testing.T) {
	chainID := "test-bbn-chain"
	backend := "test"
	homePath := t.TempDir()
	fromName := "testnode"
	cliCfg, err := config.CreateClientConfig(chainID, backend, homePath, fromName)
	require.NoError(t, err)
	...
	clientCtx, err := config.ReadFromClientConfig(
		client.Context{}.
			...
			WithInput(os.Stdin).
			WithFromName(flagFromName),
	)
	require.NoError(t, err)
	require.Equal(t, chainID, clientCtx.ChainID)
	require.Equal(t, backend, clientCtx.Keyring.Backend())
	require.Equal(t, homePath, clientCtx.HomeDir)
	require.Equal(t, fromName, clientCtx.FromName)
}
