...
Example:
	...
	cmd.Flags().String(flagStartingIPAddress, "192.168.0.1", "Starting IP address (192.168.0.1 results in persistent peers list ID0@192.168.0.1:46656, ID1@192.168.0.2:46656, ...). Alternatively it can be a hostname containing a `{i}` placeholder, turn turn templates like `node{i}.mynet` into `node0.mynet`.")
...
func calculateIP(ip string, i int) (string, error) {
	...
	if ipv4 == nil {
		if strings.Contains(ip, "{i}") {
			return strings.ReplaceAll(ip, "{i}", strconv.Itoa(i)), nil
		}
