...
BITCOINCONF=${BITCOINCONF:-/bitcoinconf}
CERTFILE="${BITCOINCONF}/rpc.cert"
KEYFILE="${BITCOINCONF}/rpc.key"

echo "Creating certificates..."
gencerts -d $BITCOINCONF -H $CLIENTHOST -f
# Copy the certificate into btcwallet
cp $CERTFILE /root/.btcwallet/btcd.cert

echo "Starting btcd..."
btcd --simnet -u $RPCUSER -P $RPCPASS --rpclisten=0.0.0.0:18556 --listen=0.0.0.0:18555 \
      --rpccert $CERTFILE --rpckey $KEYFILE 2>&1 &
BTCD_PID=$!
...
expect btcwallet_create.exp $RPCUSER $RPCPASS $PASSPHRASE $CERTFILE $KEYFILE

echo "Starting btcwallet..."
btcwallet --simnet -u $RPCUSER -P $RPCPASS --rpclisten=0.0.0.0:18554 --rpccert $CERTFILE --rpckey $KEYFILE 2>&1 &
BTCWALLET_PID=$!
...
MINING_ADDR=$(btcctl --simnet --wallet -u $RPCUSER -P $RPCPASS --rpccert $CERTFILE getnewaddress)
...
sleep 1
btcd --simnet -u $RPCUSER -P $RPCPASS --rpclisten=0.0.0.0:18556 --listen=0.0.0.0:18555 \
     --rpccert $CERTFILE --rpckey $KEYFILE --miningaddr=$MINING_ADDR 2>&1 &
BTCD_PID=$!
...
btcctl --simnet -u $RPCUSER -P $RPCPASS --rpccert $CERTFILE generate 100

# Allow some time for the wallet to catch up.
sleep 5

echo "Checking balance..."
btcctl --simnet --wallet -u $RPCUSER -P $RPCPASS --rpccert $CERTFILE getbalance

echo "Generating a block every ${GENERATE_INTERVAL_SECS} seconds."
echo "Press [CTRL+C] to stop..."
while true
do
  btcctl --simnet -u $RPCUSER -P $RPCPASS --rpccert $CERTFILE generate 1
