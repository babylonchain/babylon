...
func TestFullAppSimulation(t *testing.T) {
	...
	babylon := app.NewBabylonApp(logger, db, nil, true, map[int64]bool{}, app.DefaultNodeHome, FlagPeriodValue, app.GetEncodingConfig(), privSigner, sims.EmptyAppOptions{}, app.EmptyWasmOpts, fauxMerkleModeOpt)
...
func TestAppImportExport(t *testing.T) {
	...
	newBabylon := app.NewBabylonApp(log.NewNopLogger(), newDB, nil, true, map[int64]bool{}, app.DefaultNodeHome, FlagPeriodValue, app.GetEncodingConfig(), privSigner, sims.EmptyAppOptions{}, app.EmptyWasmOpts, fauxMerkleModeOpt)
...
func TestAppStateDeterminism(t *testing.T) {
	...
	for i := 0; i < numSeeds; i++ {
		...
		for j := 0; j < numTimesToRunPerSeed; j++ {
			...
			privSigner, err := app.SetupPrivSigner()
			require.NoError(t, err)
			babylon := app.NewBabylonApp(logger, db, nil, true, map[int64]bool{}, app.DefaultNodeHome, FlagPeriodValue, app.GetEncodingConfig(), privSigner, sims.EmptyAppOptions{}, app.EmptyWasmOpts, interBlockCacheOpt())
