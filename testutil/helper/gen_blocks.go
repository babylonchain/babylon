...
func (h *Helper) ApplyEmptyBlockWithValidBLSSig(r *rand.Rand) (sdk.Context, error) {
	emptyCtx := sdk.Context{}
	if h.App.LastBlockHeight() == 0 {
		if err := h.genAndApplyEmptyBlock(); err != nil {
			return emptyCtx, err
	...
	// 1. get vote extensions with invalid BLS signature
	blockHash := datagen.GenRandomBlockHash(r)
	evRes, err := h.App.ExtendVote(h.Ctx, &abci.RequestExtendVote{
		Hash:   blockHash,
		Height: prevHeight,
	})
	if err != nil {
	...
	valAddr := h.GenValidators.GetGenesisKeys()[0].ValPubkey.Address().Bytes()
	res, err := h.App.VerifyVoteExtension(&abci.RequestVerifyVoteExtension{
		Hash:             blockHash,
		Height:           prevHeight,
	...
	// 3. prepare proposal with previous BLS sigs
	extendedVotes := make([]abci.ExtendedVoteInfo, 0)
