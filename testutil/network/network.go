...
import (
	...
	"errors"
	"fmt"
	bbnparam "github.com/babylonchain/babylon/app/params"
...
// NewAppConstructor returns a new BabylonApp AppConstructor
func NewAppConstructor(encodingCfg bbnparam.EncodingConfig) AppConstructor {
	return func(val Validator) servertypes.Application {
		return bbnapp.NewBabylonApp(
			val.Ctx.Logger, dbm.NewMemDB(), nil, true, make(map[int64]bool), val.Ctx.Config.RootDir, 0,
			encodingCfg,
			bbnapp.EmptyAppOptions{},
...
func DefaultConfig() Config {
	encCfg := bbnapp.MakeTestEncodingConfig()
	...
	return Config{
		...
		GenesisState:      bbnapp.ModuleBasics.DefaultGenesis(encCfg.Marshaler),
...
type (
	// Network defines a local in-process testing network using BabylonApp. It can be
...
func New(t *testing.T, cfg Config) *Network {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeID, valKeys, err := genutil.InitializeNodeValidatorFiles(tmCfg)
		require.NoError(t, err)
		nodeIDs[i] = nodeID
		valPubKeys[i], err = codec2.FromTmPubKeyInterface(valKeys.ValPubkey)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		wrappedCreateValMsg := checkpointingtypes.NewMsgWrappedCreateValidator(valKeys.BlsPubkey, valKeys.PoP, createValMsg)
		...
		require.NoError(t, txBuilder.SetMsgs(wrappedCreateValMsg))
