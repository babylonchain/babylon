...
func FuzzHashesQuery(f *testing.F) {
	...
	*/
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		require.EqualError(t, err, status.Error(codes.InvalidArgument, "invalid request").Error(), "Nil input led to a nil error")
		require.Nil(t, resp, "Nil input led to a non-nil response")

		// Test request empty pagination
		resp, err = blcKeeper.Hashes(ctx, &types.QueryHashesRequest{})
		require.NoError(t, err)
		require.Equal(t, &types.QueryHashesResponse{
		...
		require.Nil(t, resp, "Invalid key led to a non-nil response")
		require.EqualError(t, err, errors.New("invalid header hash length").Error(), "Invalid key led to a nil error")
		...
		for headersRetrieved := uint64(0); headersRetrieved < chainSize; headersRetrieved += limit {
			resp, err = blcKeeper.Hashes(ctx, hashesRequest)
			require.NoError(t, err, "Valid request led to an error")
			require.NotNil(t, resp, "Valid request led to a nil response")

			// If we are on the last page the elements retrieved should be equal to the remaining ones
			if headersRetrieved+limit >= chainSize {
				require.Falsef(t, uint64(len(resp.Hashes)) != chainSize-headersRetrieved,
					"On the last page expected %d elements but got %d", chainSize-headersRetrieved, len(resp.Hashes),
				)
			}

			// Otherwise, the elements retrieved should be equal to the limit
			if headersRetrieved+limit < chainSize {
				require.Falsef(t, uint64(len(resp.Hashes)) != limit,
					"On an intermediate page expected %d elements but got %d", limit, len(resp.Hashes),
				)
			}

			for _, hash := range resp.Hashes {
				// Check if the hash was generated by the chain
				_, ok := headersMap[hash]
				require.True(t, ok, "Hashes returned a hash that was not created")
		...
		require.Equalf(t, len(hashesFound), len(headersMap), "Some hashes were missed. Got %d while %d were expected", len(hashesFound), len(headersMap))
	})
}
