...
func CreateNFinalityProviders(r *rand.Rand, t *testing.T, n int) []*types.FinalityProvider {
	fps := make([]*types.FinalityProvider, n)
	for i := 0; i < n; i++ {
		fps[i] = CreateFinalityProvider(r, t)
	}
	return fps
}
...
func CreateDelegation(
	...
	_, covenantPKs, err := datagen.GenRandomBTCKeyPairs(r, int(covenantQuorum))
	require.NoError(t, err)
	...
	var fakeCovSigs []*types.CovenantAdaptorSignatures
	var fakeUnbondingSigs []*types.SignatureInfo
	for _, covPk := range covenantPKs {
		fakeCovSigs = append(fakeCovSigs, &types.CovenantAdaptorSignatures{
			CovPk:       bbn.NewBIP340PubKeyFromBTCPK(covPk),
			AdaptorSigs: [][]byte{datagen.GenRandomByteArray(r, 64)},
		})
		fakeSig := bbn.BIP340Signature(datagen.GenRandomByteArray(r, 64))
		fakeUnbondingSigs = append(fakeUnbondingSigs, &types.SignatureInfo{
			Pk:  bbn.NewBIP340PubKeyFromBTCPK(covPk),
			Sig: &fakeSig,
		})
	...
	fakeDelegatorSig, err := bbn.NewBIP340Signature(datagen.GenRandomByteArray(r, 64))
	require.NoError(t, err)

	return &types.BTCDelegation{
		BtcPk:            bbn.NewBIP340PubKeyFromBTCPK(delPK),
		BabylonPk:        delBabylonPK.(*secp256k1.PubKey),
		Pop:              pop,
		StartHeight:      startHeight,
		EndHeight:        startHeight + uint64(stakingTime),
		...
		DelegatorSig:     fakeDelegatorSig,
		CovenantSigs:     fakeCovSigs,
		BtcUndelegation: &types.BTCUndelegation{
			SlashingTx:               testStakingInfo.SlashingTx,
			DelegatorSlashingSig:     fakeDelegatorSig,
			CovenantSlashingSigs:     fakeCovSigs,
			CovenantUnbondingSigList: fakeUnbondingSigs,
		},
	}
}
