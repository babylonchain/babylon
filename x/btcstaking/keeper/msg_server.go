...
func (ms msgServer) AddCovenantSigs(goCtx context.Context, req *types.MsgAddCovenantSigs) (*types.MsgAddCovenantSigsResponse, error) {
	...
	if btcDel.IsSignedByCovMember(pk) && btcDel.BtcUndelegation.IsSignedByCovMember(pk) {
		ms.Logger(ctx).Debug("Received duplicated covenant signature", "covenant pk", pk.MarshalHex())
		return &types.MsgAddCovenantSigsResponse{}, nil
	}

	if btcDel.HasCovenantQuorums(params.CovenantQuorum) {
		ms.Logger(ctx).Debug("Received covenant signature after achieving quorum", "covenant pk", pk.MarshalHex())
		return &types.MsgAddCovenantSigsResponse{}, nil
	}
