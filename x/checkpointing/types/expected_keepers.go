...
type CheckpointingHooks interface {
	...
	AfterRawCheckpointSealed(ctx sdk.Context, epoch uint64) error        // Must be called when a raw checkpoint is SEALED
	AfterRawCheckpointSubmitted(ctx sdk.Context, epoch uint64) error     // Must be called when a raw checkpoint is SUBMITTED
