...
`, defaultsDesc, version.AppName,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			nodeID, valKeys, err := genutil.InitializeNodeValidatorFiles(serverCtx.Config)
			if err != nil {
			...
			if pkStr, _ := cmd.Flags().GetString(cli.FlagPubKey); pkStr != "" {
				if err := clientCtx.Codec.UnmarshalInterfaceJSON([]byte(pkStr), &valKeys.ValPubkey); err != nil {
			...
			if err != nil {
				return errors.Wrap(err, "failed to convert from tm pubkey interface to cosmos pubkey interface")
			}
			createValCfg, err := cli.PrepareConfigForTxCreateValidator(cmd.Flags(), moniker, nodeID, genDoc.ChainID, cosmosValPubkey)
			if err != nil {
