...
import (
	...
	"github.com/babylonchain/babylon/x/genutil/client/cli"
	...
	for _, tc := range tests {
		...
		s.Run(tc.name, func() {
			...
			if tc.expError {
				...
				s.Require().Equal(sdk.MsgTypeURL(&checkpointingtypes.MsgWrappedCreateValidator{}), sdk.MsgTypeURL(msgs[0]))
				s.Require().True(val.Address.Equals(msgs[0].GetSigners()[0]))
				s.Require().Equal(amount, msgs[0].(*checkpointingtypes.MsgWrappedCreateValidator).MsgCreateValidator.Value)
